...
This skill file is canonical and stable. Drop it into any repo that uses Codex and the agent will pick it up automatically.

**One-liner:**

```bash
mkdir -p .Codex/skills/uianatomy-mcp \
  && curl -fsSL -o .Codex/skills/uianatomy-mcp/SKILL.md \
...
**Codex / Codex Desktop** (`.mcp.json` at project root, or `~/.config/Codex/claude_desktop_config.json`):
...
**Older clients without native streamable-HTTP** (Cursor, Windsurf, older Codex Desktop) — proxy via `mcp-remote`:
...
const transport = new StreamableHTTPClientTransport(new URL('https://uianatomy.dev/mcp'));
