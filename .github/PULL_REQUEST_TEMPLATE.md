...
- [ ] Touched a component directory? `npm run check:parity` passes with the Figma bridge open — `check:all` only _reports_ drift (ADR-0082), so this is the one place the re-verify is enforced.
