...
const entries = Object.values(idx.entries || {}).filter(
  (e) => e.type !== 'docs',
);
...
for (const [i, s] of picked.entries()) {
  ...
  const page = await b.newPage({
    viewport: { width: wide ? 1280 : 720, height: wide ? 1400 : 320 },
    deviceScaleFactor: 2,
  });
  const errs = [];
  page.on('console', (m) => {
  ...
  await page.goto(
    `http://localhost:${PORT}/iframe.html?id=${s.id}&viewMode=story`,
