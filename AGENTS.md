...
**Toolset gating** (addon-mcp options, all default `true`): `docs` requires the `componentsManifest` feature flag — addon-mcp's own preset switches it on, and it is the flag core-server reads when writing the manifest — plus an actually emitted `components.json`. React gets this for free (`react-docgen` is its default docgen path); Angular and Vue need `experimentalDocgenServer` — the default under `@storybook/angular-vite`, opt-in until Storybook 11 for `@storybook/vue3-vite` — and this repo sets it explicitly for both rather than resting on the current default. Inside `dev`, `stories-preview` and `get-storybook-story-instructions` need nothing extra; `stories-changed` and `review-create` need `features.changeDetection` (10.4's Change Review sidebar, which defaults to **on** — none of this repo's three `main.ts` overrides it). **`review-create` is the wire name**: the tool registers under the method id `review.create`, so `tools/list` returns `review-create`; `display-review` is Storybook's changelog prose for it and matches nothing you can call. Its `experimentalReview` gate is channel-dependent rather than a flat default — unset, it enables the tool only for the deprecated `storybook ai` CLI proxy channel, leaving a direct MCP client (a local `.mcp.json` dev entry) without `review-create` unless `experimentalReview: true` is set explicitly (`common-preset.js`'s `isReviewExplicitlyEnabled` vs. `isReviewFeatureEnabled`). Re-verified against the installed 10.6.0 dist, 2026-09-13 — the claim still held, so this repo now sets `experimentalReview: true` explicitly in all three `main.ts` and in the `create-workspace` scaffold's three templates (plan/adr/0143), and a direct MCP client reaches `review-create` too, not only the CLI proxy channel. `stories-find-by-component` needs a builder that exposes the module-graph service. `test` requires `@storybook/addon-vitest`; a11y in `test-run` activates when `@storybook/addon-a11y` is installed.
