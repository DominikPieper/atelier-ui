...
- **Fonts.** `assets/colors_and_type.css` is a generated 1:1 copy of the upstream `libs/react/src/styles/tokens.css` (written by `tools/scripts/sync-tokens.mjs`, gated by `check:tokens`) and intentionally does **not** load any web font — the Astro docs site supplies Instrument Sans + Instrument Serif + JetBrains Mono through its own pipeline. When using this file standalone (slides, throwaway artifacts), prepend `@import url('https://fonts.googleapis.com/css2?family=Instrument+Sans:wght@400;500;600;700&family=Instrument+Serif:ital@1&family=JetBrains+Mono:wght@400;500;600&display=swap');`. The sheet already declares `--ui-font-family`, `--ui-font-display` and `--ui-font-mono`, so nothing needs adding to `:root`. To vendor fonts, drop `.woff2` files into a `fonts/` folder and swap the import.
