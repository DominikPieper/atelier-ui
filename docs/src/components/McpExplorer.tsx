...
const TOOL_DEFS: ToolDef[] = [
    ...
    params: [
      {
        ...
        suggestions: ALL_COMPONENT_SLUGS,
      },
    ],
  ...
  },
  {
    ...
    workshopTip:
      'Called at the start of a session or when the user asks "what components are available?". Gives the AI a full map of the library before diving into specifics.',
    params: [],
    defaultParams: {},
  },
  {
    ...
    description:
      'Fuzzy-search components by keyword. Returns matching names and descriptions.',
    workshopTip:
      'Called when the user expresses an intent rather than naming a specific component. Helps the AI match need to component.',
    params: [
      {
        name: 'query',
        type: 'string',
  ...
  },
  {
    ...
    signature: '(component: string)',
    description:
      'Returns Storybook story metadata for a component — names, descriptions, and variant previews.',
    workshopTip:
      'Called when the user wants usage examples. The AI reads the story names and descriptions to show real-world patterns.',
    params: [
      {
        name: 'component',
        type: 'string',
  ...
  },
  {
    ...
    signature: '()',
    description:
      'Returns the full CSS custom property reference, token categories, and dark mode setup.',
    workshopTip:
      'Called when the user asks about customising colors, spacing, or dark mode. The AI gets the exact token names and values — no hallucinated CSS variable names.',
    params: [],
    defaultParams: {},
  },
];
...
const MOCK_RESPONSES: Record<
  string,
  (params: Record<string, string>, fw: Framework) => object
> = {
  get_component_docs: (params, fw) => {
    ...
    if (fw === 'angular') {
      return {
        ...
        inputs: {
          variant:
            "'primary' | 'secondary' | 'outline' | 'danger' (default: 'primary')",
    ...
    if (fw === 'react') {
      return {
        component: comp,
        element: `Atl${comp.charAt(0).toUpperCase() + comp.slice(1)}`,
  ...
  get_stories: (params, fw) => ({
    ...
    stories: [
      ...
      {
        name: 'Variants',
        description: 'primary, secondary, outline, danger side by side',
      },
  ...
  get_theming_guide: () => ({
    ...
    dark_mode:
      'Set data-theme="dark" on <html> — all tokens switch automatically',
  }),
};
...
export default function McpExplorer() {
  ...
  const activeMockTool =
    TOOL_DEFS.find((t) => t.name === selectedTool) ?? TOOL_DEFS[0];
  ...
  function handleCall() {
    ...
    requestAnimationFrame(() =>
      requestAnimationFrame(() => setRequestVisible(true)),
    );
    ...
    if (serverOnline) {
      invokeMcpTool(framework, selectedTool, params)
        .then((result) => {
          ...
          setCalling(false);
          requestAnimationFrame(() =>
            requestAnimationFrame(() => setResponseVisible(true)),
          );
        })
        .catch((err) => {
  ...
  const activeParams = activeLiveTool
    ? Object.entries(activeLiveTool.inputSchema.properties ?? {}).map(
        ([name, p]) => ({
          name,
          type: p.type,
  ...
  return (
    <div style={{ marginTop: '2.5rem' }}>
      ...
      <div style={{ marginBottom: '2.5rem' }}>
        <h2 className="docs-section-title" style={{ marginTop: 0 }}>
          Try a tool
        </h2>
        <p
          style={{
        ...
        >
          Pick a framework, select a tool, set parameters, and call it to see
          exactly what structured data an AI receives. The tools here are a
          simplified conceptual model — the real hosted servers expose the
      ...
      >
        ...
        >
          <strong style={{ color: 'var(--ui-color-text)' }}>Note:</strong> The
          hosted endpoints expose the docs toolset only — for all three
          frameworks. Angular and Vue answer component lookups from their own
          components manifest (angular-component-meta, vue-component-meta;
          ADR-0097), the same as React's (react-docgen). Variants, defaults and
          state props come back shaped for the framework you asked: two-way
          [(checked)] bindings and split Inputs/Outputs for Angular,
          v-model/update:* events and typed slots for Vue, unchanged
          JSX/children for React. Story previews and test tools come from the
          local @storybook/addon-mcp behind a running Storybook — all three
          frameworks, not just React, since Storybook 10.6.
        </p>
      </div>

      {/* Protocol flow */}
      <div
        className="mcp-protocol-steps"
        style={{
          background: 'var(--ui-color-surface-sunken)',
          borderRadius: 'var(--ui-radius-md)',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                      ...
                      {param.suggestions.slice(0, 12).map((s) => {
                        ...
                        return (
                          <button
                            ...
                            aria-label={`Set ${param.name} to ${s}`}
                            onClick={() => {
          ...
          <div>
            <button
              ...
              disabled={calling}
              aria-label={
                calling ? `Calling ${selectedTool}` : `Call ${selectedTool}`
              }
              style={{
                ...
                letterSpacing: '0.03em',
                animation: calling
                  ? 'mcp-pulse 1.4s ease-in-out infinite'
                  : 'none',
              }}
            >
          ...
          {request && (
            <div
              style={{
                ...
                transition:
                  'opacity var(--ui-transition-normal), transform var(--ui-transition-normal)',
              }}
            >
              <CodePane
          ...
          {response && (
            <div
              style={{
                ...
                transform: responseVisible
                  ? 'translateY(0)'
                  : 'translateY(6px)',
                transition:
                  'opacity var(--ui-transition-slow), transform var(--ui-transition-slow)',
              }}
            >
              <CodePane
              ...
              >
                <strong
                  style={{ color: 'var(--ui-color-text)', fontWeight: 700 }}
                >
                  What the AI does next:
                </strong>{' '}
                this JSON is injected into the model's context. It reads the
                exact field names, types, and defaults — and uses them verbatim
                when generating code. No guessing, no hallucinated prop names.
              </div>
            </div>
          )}
        </div>
      ...
      >
        ...
        >
          The MCP servers are live. Add them to your AI tool's config and every
          tool shown above works directly — Claude, Cursor, and any
          MCP-compatible host.
        </p>
        <div
          style={{
