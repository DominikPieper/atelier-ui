...
@Component({
  ...
  providers: [{ provide: ATL_ACCORDION_GROUP, useExisting: AtlAccordionGroup }],
})
export class AtlAccordionGroup implements AtlAccordionGroupContext {
 ...
 */
@Directive({
  selector: '[atlAccordionHeader]',
  standalone: true,
})
export class AtlAccordionHeader {}
...
export class AtlAccordionItem implements AccordionItem, OnInit, OnDestroy {
  ...
  /** @internal */
  readonly id = `atl-accordion-item-${nextId++}`;
  ...
  /** @internal */
  private readonly group = inject(AtlAccordionGroup);
