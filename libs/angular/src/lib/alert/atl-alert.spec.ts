...
describe('AtlAlert', () => {
  ...
  it('has role="alert" on host', async () => {
    const { container } = await render('<atl-alert>Message</atl-alert>', {
      imports: [AtlAlert],
    });
    expect(container.querySelector('atl-alert')).toHaveAttribute(
      'role',
      'alert',
    );
  });
  ...
  describe('variant classes', () => {
    it.each(['info', 'success', 'warning', 'danger'] as const)(
      ...
      async (variant) => {
        ...
        expect(container.querySelector('atl-alert')).toHaveClass(
          `variant-${variant}`,
        );
      },
    );
  ...
  describe('aria-live', () => {
    covers('alert', 'aria-live')(
      'sets aria-live="polite" for non-danger variants',
      async () => {
        const { container } = await render(
  ...
  describe('dismiss button', () => {
    covers('alert', 'dismiss-hidden')(
      'does not render dismiss button when dismissible=false',
      async () => {
    ...
    covers('alert', 'dismiss-shown')(
      ...
      async () => {
        await render('<atl-alert [dismissible]="true">Message</atl-alert>', {
          imports: [AtlAlert],
        });
        expect(
    ...
    covers('alert', 'emits-dismiss')(
      'emits dismissed output when dismiss button is clicked',
      async () => {
        const user = userEvent.setup();
        const dismissed = vi.fn();
        await render(
          '<atl-alert [dismissible]="true" (dismissed)="dismissed()">Message</atl-alert>',
          {
            imports: [AtlAlert],
