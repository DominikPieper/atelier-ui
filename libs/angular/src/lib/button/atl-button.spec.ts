...
describe('AtlButton', () => {
  ...
  describe('variant classes', () => {
    it.each(['primary', 'secondary', 'outline', 'danger'] as const)(
      ...
      async (variant) => {
        ...
        expect(container.querySelector('atl-button')).toHaveClass(
          `variant-${variant}`,
        );
      },
    );
  ...
  describe('disabled state', () => {
    covers('button', 'disabled-state')(
      ...
      async () => {
        ...
        expect(container.querySelector('atl-button')).toHaveAttribute(
          'aria-disabled',
          'true',
        );
      },
  ...
  it('projects slotted content into the button', async () => {
    await render('<atl-button>Save Changes</atl-button>', {
      imports: [AtlButton],
    });
