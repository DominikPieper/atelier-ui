 ...
 *
 * Angular renders an `<llm-button role="button">` host (not a native button), so
...
async function captureOne(template: string): Promise<unknown> {
  ...
  const r = await render(template, { imports: [LlmButton] });
...
async function capture(): Promise<Record<string, unknown>> {
  return {
    default: await captureOne('<llm-button>Click me</llm-button>'),
    disabled: await captureOne('<llm-button [disabled]="true">Click me</llm-button>'),
    loading: await captureOne('<llm-button [loading]="true">Click me</llm-button>'),
  };
}
