...
const TEMPLATE = `<atl-code-block [code]="code" [language]="language" [filename]="filename" [copyable]="copyable" [showLineNumbers]="showLineNumbers" />`;
...
describe('AtlCodeBlock', () => {
  ...
  covers('code-block', 'copy-button')(
    'renders a copy button when copyable is true',
    async () => {
      await render(TEMPLATE, {
        imports: [AtlCodeBlock],
        componentProperties: defaults(),
