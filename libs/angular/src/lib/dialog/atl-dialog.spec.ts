...
describe('AtlDialog', () => {
  ...
  covers('dialog', 'render-dialog-element')(
  ...
  it('does not have open attribute when open=false', async () => {
    const { container } = await render(
      '<atl-dialog [open]="false">Content</atl-dialog>',
      {
        imports: [AtlDialog],
      },
    );
  ...
  covers('dialog', 'open-shows-modal')(
    'opens the dialog when open=true',
    async () => {
      const { container, fixture } = await render(
        '<atl-dialog [open]="true">Content</atl-dialog>',
        {
          imports: [AtlDialog],
        },
      );
  ...
  describe('size variants', () => {
    for (const size of ['sm', 'md', 'lg', 'xl', 'full'] as const) {
      covers('dialog', 'size-class')(
        `applies size-${size} class to .panel`,
        async () => {
          const { container } = await render(
  ...
  describe('close button', () => {
    ...
    covers('dialog', 'close-button')(
      'closes the dialog when close button is clicked',
      async () => {
        const user = userEvent.setup();
        const { container } = await render(
        ...
        expect(container.querySelector('atl-dialog')).not.toHaveClass(
          'is-open',
        );
      },
    );
