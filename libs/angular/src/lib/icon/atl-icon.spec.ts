...
describe('AtlIcon', () => {
  covers('icon', 'renders-geometry')(
    ...
    async () => {
      const { container } = await render('<atl-icon name="success" />', {
        imports: [AtlIcon],
      });
      const paths = [...container.querySelectorAll('atl-icon svg path')].map(
        (p) => p.getAttribute('d'),
      );
  ...
  it.each(Object.keys(ATL_ICON_GEOMETRY))(
    'draws at least one path for %s',
    async (name) => {
      const { container } = await render(`<atl-icon name="${name}" />`, {
        imports: [AtlIcon],
      });
      expect(
        container.querySelectorAll('atl-icon svg path').length,
      ).toBeGreaterThan(0);
    },
  );
  ...
  it('paints stroke icons with currentColor and no fill', async () => {
    const { container } = await render('<atl-icon name="close" />', {
      imports: [AtlIcon],
    });
  ...
  it('paints fill icons with currentColor and no stroke', async () => {
    const { container } = await render('<atl-icon name="person" />', {
      imports: [AtlIcon],
    });
    const svg = container.querySelector('svg');
  ...
  covers('icon', 'decorative-hidden')(
    'is hidden from assistive tech by default (decorative)',
    async () => {
      const { container } = await render('<atl-icon name="info" />', {
        imports: [AtlIcon],
      });
  ...
  covers('icon', 'labelled-img')(
    'exposes role=img and aria-label when label is provided',
    async () => {
