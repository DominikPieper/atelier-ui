...
async function captureOne(template: string): Promise<unknown> {
  // Reset between renders so multiple scenarios can run in one test
  // (TestBed throws if reconfigured while already instantiated).
  TestBed.resetTestingModule();
  const r = await render(template, { imports: [AtlInput] });
