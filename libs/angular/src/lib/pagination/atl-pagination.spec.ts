...
describe('AtlPagination', () => {
  ...
  it('renders prev and next buttons', async () => {
    await render('<atl-pagination [pageCount]="5" />', {
      imports: [AtlPagination],
    });
    expect(
  ...
  it('renders first and last buttons when showFirstLast=true', async () => {
    await render('<atl-pagination [pageCount]="5" [showFirstLast]="true" />', {
      imports: [AtlPagination],
    });
    expect(
  ...
  covers('pagination', 'hide-first-last')(
    'does not render first/last buttons when showFirstLast=false',
    async () => {
      await render(
        '<atl-pagination [pageCount]="5" [showFirstLast]="false" />',
        {
          imports: [AtlPagination],
        },
      );
  ...
  covers('pagination', 'disables-prev-first')(
    'disables prev/first buttons on page 1',
    async () => {
      await render('<atl-pagination [page]="1" [pageCount]="5" />', {
        imports: [AtlPagination],
      });
      expect(
        screen.getByRole('button', { name: 'Previous page' }),
  ...
  covers('pagination', 'disables-next-last')(
    ...
    async () => {
      await render('<atl-pagination [page]="5" [pageCount]="5" />', {
        imports: [AtlPagination],
      });
  ...
  covers('pagination', 'aria-current')(
    'sets aria-current="page" on the active page button',
    async () => {
      await render('<atl-pagination [page]="3" [pageCount]="5" />', {
        imports: [AtlPagination],
      });
  ...
  describe('pageItems algorithm', () => {
    it('shows all pages when no ellipsis needed (small count)', async () => {
      const { container } = await render(
        '<atl-pagination [page]="2" [pageCount]="3" [showFirstLast]="false" />',
        { imports: [AtlPagination] },
      );
      const pageBtns = container.querySelectorAll(
        '.page-btn:not([aria-label="Previous page"]):not([aria-label="Next page"])',
      );
    ...
    it('shows left ellipsis when current is far right', async () => {
      const { container } = await render(
        '<atl-pagination [page]="9" [pageCount]="10" [siblingCount]="1" [showFirstLast]="false" />',
        { imports: [AtlPagination] },
      );
    ...
    it('shows right ellipsis when current is far left', async () => {
      const { container } = await render(
        '<atl-pagination [page]="1" [pageCount]="10" [siblingCount]="1" [showFirstLast]="false" />',
        { imports: [AtlPagination] },
      );
      const ellipsis = container.querySelectorAll('.ellipsis');
      expect(ellipsis).toHaveLength(1);
    ...
    it('shows both ellipses when current is in the middle', async () => {
      const { container } = await render(
        '<atl-pagination [page]="5" [pageCount]="10" [siblingCount]="1" [showFirstLast]="false" />',
        { imports: [AtlPagination] },
      );
      const ellipsis = container.querySelectorAll('.ellipsis');
    ...
    it('ellipsis elements are aria-hidden', async () => {
      const { container } = await render(
        '<atl-pagination [page]="5" [pageCount]="10" [siblingCount]="1" />',
        { imports: [AtlPagination] },
      );
  ...
  describe('navigation', () => {
    it('increments page on next click', async () => {
      ...
      const { container } = await render(
        '<atl-pagination [(page)]="page" [pageCount]="5" />',
        { imports: [AtlPagination], componentProperties: { page: 2 } },
      );
    ...
    it('decrements page on prev click', async () => {
      ...
      const { container } = await render(
        '<atl-pagination [(page)]="page" [pageCount]="5" />',
        { imports: [AtlPagination], componentProperties: { page: 3 } },
      );
    ...
    it('jumps to first page on first button click', async () => {
      ...
      const { container } = await render(
        '<atl-pagination [(page)]="page" [pageCount]="5" />',
        { imports: [AtlPagination], componentProperties: { page: 4 } },
      );
    ...
    covers('pagination', 'page-change-on-click')(
      'navigates to a specific page on click',
      async () => {
        const user = userEvent.setup();
        const { container } = await render(
          '<atl-pagination [(page)]="page" [pageCount]="10" [siblingCount]="2" />',
          { imports: [AtlPagination], componentProperties: { page: 1 } },
        );
        await user.click(screen.getByRole('button', { name: 'Page 3' }));
        expect(
          container.querySelector('.page-btn.is-active'),
        ).toHaveTextContent('3');
      },
    );
  });
