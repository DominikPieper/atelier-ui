...
describe('AtlPagination', () => {
  it('renders a nav with aria-label="Pagination"', async () => {
    const { container } = await render('<atl-pagination />', { imports: [AtlPagination] });
  ...
  it('renders prev and next buttons', async () => {
    await render('<atl-pagination [pageCount]="5" />', { imports: [AtlPagination] });
  ...
  covers('pagination', 'hide-first-last')('does not render first/last buttons when showFirstLast=false', async () => {
    await render('<atl-pagination [pageCount]="5" [showFirstLast]="false" />', {
      imports: [AtlPagination],
    });
