...
describe('AtlProgress', () => {
  it('has role="progressbar"', async () => {
    ...
    expect(container.querySelector('atl-progress')).toHaveAttribute(
      'role',
      'progressbar',
    );
  });
  ...
  it('sets aria-valuemax to the max', async () => {
    const { container } = await render(
      '<atl-progress [max]="200" [value]="100" />',
      {
        imports: [AtlProgress],
      },
    );
  ...
  describe('variant classes', () => {
    for (const variant of [
      ...
      it(`applies variant-${variant} class`, async () => {
        ...
        expect(container.querySelector('atl-progress')).toHaveClass(
          `variant-${variant}`,
        );
      });
    }
  ...
  describe('indeterminate', () => {
    ...
    covers('progress', 'indeterminate-omits-valuenow')(
      'does not set aria-valuenow when indeterminate',
      async () => {
        const { container } = await render(
          '<atl-progress [indeterminate]="true" />',
          {
        ...
        expect(container.querySelector('atl-progress')).not.toHaveAttribute(
          'aria-valuenow',
        );
      },
    );
    ...
    it('does not add is-indeterminate class by default', async () => {
      ...
      expect(container.querySelector('atl-progress')).not.toHaveClass(
        'is-indeterminate',
      );
    });
  });
