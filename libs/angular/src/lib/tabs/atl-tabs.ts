...
export class AtlTabGroup implements AtlTabGroupContext {
...
export class AtlTab implements OnInit, OnDestroy {
  ...
  /** @internal */
  readonly tabId = `atl-tab-${nextId}`;

  /** @internal */
  readonly panelId = `atl-tab-panel-${nextId}`;

  /** @internal */
  private readonly id = `atl-tab-instance-${nextId++}`;

  /** @internal */
  private readonly context = inject(ATL_TAB_GROUP);
