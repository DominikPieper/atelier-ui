...
const SNAP = resolve(ROOT, `tools/parity/a11y/atl-checkbox.${FW}.json`);

function capture() {
  const scenarios: Record<string, unknown> = {};
  let r = render(<AtlCheckbox>Accept terms</AtlCheckbox>);
  scenarios.unchecked = a11yTree(r.container);
  r.unmount();
  r = render(<AtlCheckbox checked>Accept terms</AtlCheckbox>);
  scenarios.checked = a11yTree(r.container);
  r.unmount();
  r = render(<AtlCheckbox disabled>Accept terms</AtlCheckbox>);
  scenarios.disabled = a11yTree(r.container);
  r.unmount();
  return scenarios;
}

describe('AtlCheckbox — a11y conformance snapshot', () => {
