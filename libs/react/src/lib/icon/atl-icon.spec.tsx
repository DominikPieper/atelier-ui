...
describe('AtlIcon', () => {
  ...
  it.each(Object.keys(ATL_ICON_GEOMETRY))(
    ...
    (name) => {
      const { container } = render(
        <AtlIcon name={name as keyof typeof ATL_ICON_GEOMETRY & never} />,
      );
      expect(
  ...
  it('paints stroke icons with currentColor and no fill, fill icons the other way round', () => {
    const stroke = render(<AtlIcon name="close" />).container.querySelector(
      'svg',
    );
    expect(stroke).toHaveAttribute('fill', 'none');
    expect(stroke).toHaveAttribute('stroke', 'currentColor');

    const filled = render(<AtlIcon name="person" />).container.querySelector(
      'svg',
    );
  ...
  covers('icon', 'decorative-hidden')(
    'is hidden from assistive tech by default',
    () => {
