...
describe('AtlPagination', () => {
  ...
  covers('pagination', 'aria-current')(
    'marks the current page button with aria-current="page"',
    () => {
  ...
  covers('pagination', 'hide-first-last')(
    'does not render First and Last buttons when showFirstLast is false',
    () => {
  ...
  covers('pagination', 'disables-prev-first')(
    'disables Previous and First buttons on page 1',
    () => {
      render(<AtlPagination page={1} pageCount={5} />);
      expect(
        screen.getByRole('button', { name: 'Previous page' }),
  ...
  covers('pagination', 'disables-next-last')(
    'disables Next and Last buttons on last page',
    () => {
  ...
  it('calls onPageChange with next page when Next is clicked', async () => {
    ...
    render(
      <AtlPagination page={2} pageCount={5} onPageChange={onPageChange} />,
    );
  ...
  it('calls onPageChange with previous page when Previous is clicked', async () => {
    ...
    const onPageChange = vi.fn();
    render(
      <AtlPagination page={3} pageCount={5} onPageChange={onPageChange} />,
    );
  ...
  covers('pagination', 'page-change-on-click')(
    'calls onPageChange with specific page when page button is clicked',
    async () => {
      const user = userEvent.setup();
      const onPageChange = vi.fn();
      render(
        <AtlPagination page={1} pageCount={3} onPageChange={onPageChange} />,
      );
  ...
  it('calls onPageChange with 1 when First is clicked', async () => {
    ...
    const onPageChange = vi.fn();
    render(
      <AtlPagination page={4} pageCount={5} onPageChange={onPageChange} />,
    );
