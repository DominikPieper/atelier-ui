...
function TestHarness({
  ...
  return (
    <>
      ...
      <button onClick={() => show('Danger!', { variant: 'danger' })}>
        Show danger
      </button>
      <button onClick={() => show('No dismiss', { dismissible: false })}>
...
describe('AtlToast', () => {
  covers('toast', 'show-adds')(
    'shows a toast when show() is called',
    async () => {
  ...
  covers('toast', 'variant-class')(
    'shows toast with success variant class',
    async () => {
      const user = userEvent.setup();
      renderWithProvider();
      await user.click(screen.getByRole('button', { name: 'Show success' }));
      expect(screen.getByText('Success!').closest('.atl-toast')).toHaveClass(
        'variant-success',
      );
    },
  );
  ...
  it('shows toast with danger variant class', async () => {
    ...
    expect(screen.getByText('Danger!').closest('.atl-toast')).toHaveClass(
      'variant-danger',
    );
  });
  ...
  it('does not show dismiss button when dismissible is false', async () => {
    ...
    renderWithProvider();
    await user.click(
      screen.getByRole('button', { name: 'Show non-dismissible' }),
    );
    expect(
  ...
  covers('toast', 'dismiss-button-click')(
    'dismisses toast when dismiss button is clicked',
    async () => {
      const user = userEvent.setup();
      renderWithProvider();
      await user.click(screen.getByRole('button', { name: 'Show default' }));
  ...
  covers('toast', 'position-class')('container applies position class', () => {
    ...
    expect(document.querySelector('.atl-toast-container')).toHaveClass(
      'position-top-center',
    );
  });
  ...
  it.each([
    ...
    'bottom-right',
    'bottom-center',
  ] as const)('applies position-%s class', (position) => {
    render(
      <AtlToastProvider>
        <AtlToastContainer position={position} />
      </AtlToastProvider>,
