...
interface AtlAccordionItemProps {
  expanded?: boolean;
  disabled?: boolean;
  /**
...
const props = withDefaults(defineProps<AtlAccordionItemProps>(), {
  expanded: undefined,
  disabled: false,
  headingLevel: 3,
});
...
const id = useId();
const triggerId = `atl-accordion-trigger-${id}`;
const panelId = `atl-accordion-panel-${id}`;

const group = inject(AtlAccordionGroupKey, null);
