...
describe('AtlAccordionGroup', () => {
  ...
  covers('accordion', 'collapse-on-click')(
    'collapses an open item on second click',
    async () => {
      const user = userEvent.setup();
      render(AccordionFixture);
      const btn = screen.getByRole('button', { name: /Question 1/i });
  ...
  covers('accordion', 'single-collapse-other')(
    'collapses previously open item when multi is false',
    async () => {
      const user = userEvent.setup();
      render(AccordionFixture);
  ...
  covers('accordion', 'multi-expand')(
    'allows multiple open items when multi is true',
    async () => {
      const user = userEvent.setup();
      render({
        components: { AtlAccordionGroup, AtlAccordionItem },
  ...
  covers('accordion', 'disabled-no-toggle')(
    'does not expand a disabled item',
    async () => {
      render({
        components: { AtlAccordionGroup, AtlAccordionItem },
        template: `
