...
function captureOne(props: Record<string, unknown>, text: string): unknown {
  const r = render(AtlBadge, { props, slots: { default: text } });
...
function capture() {
  return {
    default: captureOne({}, 'New'),
    success: captureOne({ variant: 'success' }, 'Active'),
  };
}
