...
describe('AtlChat', () => {
  describe.each(['drawer', 'popup', 'inline'] as const)(
    'variant=%s',
    (variant) => {
      covers('chat', 'variant-class')(
        'applies the variant class on the host',
        () => {
          const { container } = render(ChatFixture, {
            props: { variant, status: 'idle', open: true },
          });
  ...
  describe.each(['idle', 'streaming', 'error'] as const)(
    'status=%s',
    (status) => {
      it('applies the status class on the host', () => {
        const { container } = render(ChatFixture, {
          props: { variant: 'inline', status, open: true },
        });
  ...
  covers('chat', 'inline-variant')(
    'inline variant renders a section, no dialog or fab',
    () => {
      const { container } = render(ChatFixture, {
        props: { variant: 'inline', status: 'idle', open: true },
      });
      expect(
  ...
  covers('chat', 'popup-variant')(
    'popup variant renders the floating bubble + popup window',
    () => {
      const { container } = render(ChatFixture, {
        props: { variant: 'popup', status: 'idle', open: true },
      });
  ...
  it('drawer variant renders a dialog and opens it via showModal', async () => {
    const { container, rerender } = render(ChatFixture, {
      props: { variant: 'drawer', status: 'idle', open: false },
    });
  ...
  describe('AtlChatMessage', () => {
    it.each(['user', 'assistant', 'system'] as const)(
      ...
      (role) => {
        const { container } = render(AtlChatMessage, {
          props: { role },
          slots: { default: 'Hi' },
        });
  ...
  describe('AtlChatSuggestion', () => {
    it('emits selected with the label when clicked', async () => {
      ...
      const { emitted } = render(AtlChatSuggestion, {
        props: { label: 'Explain' },
      });
  ...
  describe('AtlChatInput', () => {
    ...
    covers('chat', 'send-button-idle')(
      ...
      () => {
        const { container } = render(InputFixture, {
          props: { status: 'idle' },
        });
        expect(
    ...
    covers('chat', 'emits-stop')(
      'emits stop when Stop is clicked while streaming',
      async () => {
        const user = userEvent.setup();
        const { emitted } = render(InputFixture, {
          props: { status: 'streaming' },
        });
