...
describe('AtlCheckbox', () => {
  ...
  covers('checkbox', 'toggle-emits')(
    'emits update:checked on change',
    async () => {
      const user = userEvent.setup();
      const { emitted } = render(AtlCheckbox, {
        props: { checked: false },
  ...
  covers('checkbox', 'invalid')(
    'marks input as invalid when invalid prop is true',
    () => {
      render(AtlCheckbox, {
        props: { invalid: true },
  ...
  covers('checkbox', 'indeterminate')(
    'sets the indeterminate property on the input when indeterminate is true',
    () => {
      ...
      expect(
        (container.querySelector('input') as HTMLInputElement).indeterminate,
      ).toBe(true);
    },
  );
