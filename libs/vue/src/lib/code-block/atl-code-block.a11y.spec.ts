...
const SNAP = resolve(ROOT, `tools/parity/a11y/atl-code-block.${FW}.json`);

function captureOne(props: Record<string, unknown>): unknown {
  const r = render(AtlCodeBlock, { props });
...
function capture() {
  return {
    default: captureOne({ code: 'const x = 1;', language: 'ts' }),
  };
}

describe('AtlCodeBlock — a11y conformance snapshot', () => {
