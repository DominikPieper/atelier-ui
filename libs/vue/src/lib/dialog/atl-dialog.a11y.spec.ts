...
const SNAP = resolve(ROOT, `tools/parity/a11y/atl-dialog.${FW}.json`);

// jsdom does not implement showModal/close natively; polyfill so the open
// state reaches the DOM (and thus the captured accessibility tree).
beforeEach(() => {
...
function capture() {
  ...
  const r = render(AtlDialog, {
    props: { open: true, ariaLabel: 'Confirm' },
...
describe('AtlDialog — a11y conformance snapshot', () => {
