...
describe('AtlSkeleton', () => {
  ...
  covers('skeleton', 'not-animated')(
    'does not add is-animated class when animated is false',
    () => {
  ...
  covers('skeleton', 'custom-size')(
    ...
    () => {
      const { container } = render(AtlSkeleton, {
        props: { variant: 'rectangular', width: '200px', height: '50px' },
      });
      const el = container.firstChild as HTMLElement;
  ...
  covers('skeleton', 'circular-height')(
    'computes height for circular variant to match width',
    () => {
      const { container } = render(AtlSkeleton, {
        props: { variant: 'circular', width: '48px' },
      });
      const el = container.firstChild as HTMLElement;
