...
tokens) — ADR-0004 decides _that_ all visuals flow through `--ui-*` tokens; this ADR decides
_how the token tiers are structured in Figma_ and _which tiers cross over into code_.
...
1. **Three Figma tiers: Primitive → UI (semantic) → Component, one alias hop per tier.**
   Why: separates _value_ (Primitive) from _meaning_ (UI) from _component-local role_ (Component),
...
2. **Modes (Light/Dark) live only on the UI tier.**
   ...
   stay single-mode because a Primitive with per-theme values _is_ a misused semantic.
...
3. **The Component tier exists in Figma but is NOT projected into CSS.**
   ...
   `button/bg-primary`, not the abstract `color/primary`), and it makes the third tier _visible_ for
