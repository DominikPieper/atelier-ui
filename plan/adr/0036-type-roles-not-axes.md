...
_weight comes from the sans; the display face ships one weight and must never be
bolded_ — existed only as prose in the ADR. Nothing stopped a component from
...
once the rule to encode is a _relationship_ between family, style and weight.
...
- **`font:` is a shorthand, with the footgun that implies.** It resets
  ...
  declared _before_ any `font-*` override in the same rule. Documented in the
