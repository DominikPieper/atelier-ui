...
`--ui-color-input-bg` already _is_ `var(--ui-color-surface-sunken)`, and
...
attribute _and_ fired `onCheckedChange`, so they reported the change the prop
...
spec file into each lib; `check:defaults` compares default _values_ for axis props
...
- **Vue loses two working guards** (toggle, select). That is the real cost of this
  option: those two _could_ enforce it, as Vue proved. Chosen anyway, because the
...
and dark. Rejected: a lighter background (a white field reads as _more_ editable in
