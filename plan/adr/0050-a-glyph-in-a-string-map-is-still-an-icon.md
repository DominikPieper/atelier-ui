...
signal status were still drawing their signals as text, _after_ the ADR that was
...
_names_ — `info` / `success` / `warning` / `danger`, which the geometry already
...
holds only because the glyph is purely decorative _and_ was made to verifiably
...
- **This came out of drawing, not of scanning.** The glyphs had been sitting in the
  ...
  on a sheet, what the alert's icon _is_.
