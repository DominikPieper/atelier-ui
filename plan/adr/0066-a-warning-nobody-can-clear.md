...
AtlMenuSeparator, AtlChatMessage, AtlChatSuggestion, AtlChatTyping — each saying _"no
spec/registry mapping, parity inputs cannot be located. Add it to
COMPONENT_METADATA_REGISTRY or allowlist it."_ There is no allowlist for that check, and
...
metadata module _"inherits false claims"_ and that the children should get their own
...
`DOCS_PRIMARY_SPECS` states that _"one docs entry documents one primary interface"_ and
that child interfaces are _"intentionally absent"_, and nine modules cover several specs
that way. `NON_COMPONENT_SPECS` lists `AtlChatMessageSpec` and `AtlChatSuggestionSpec`
explicitly as _"shape[s] rendered by AtlChatSpec"_. The `radio` module even lists the
...
- **No such spec interface at all** — AtlToast and AtlCodeBlock have no contract by
  ...
  AtlChatTyping earn a master by being _placeable_ rather than by having state
...
the matrix pictures the select _trigger_. The fix is a field on the type
...
**3. That allowlist entry's reason was corrected.** It said _"a child spec needs its own
metadata module; tracked in tasks/todo.md"_ — repeating the wrong premise. An exemption's
