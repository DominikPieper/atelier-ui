...
| Block | Zeit | Format | Inhalt | Quellen / MCP / Skill |
|-------|------|--------|--------|-----------------------|
| **0. Recap & Brief** | 09:00–09:30 (30m) | Vortrag | Tag-1-Recap, Komponenten-Brief vorstellen (Anatomie, Axes, States, A11y-Anforderungen) — `workshop/briefs/<komponente>.md`. `uianatomy` MCP als Hilfsmittel zeigen; zwei der vier Briefs sind direkt daraus generiert, zwei sind Kompositionen, weil die Komponente im kanonischen Katalog fehlt. Zugleich das **Skills-Konzept** am lebenden Beispiel: ein Skill ist eine Anleitung, die Claude bei passender Aufgabe lädt — `uianatomy-mcp` triggert hier automatisch, `figma-workspace-architect` und `atelier-design` begleiten Block 1. | `workshop/briefs/`, `plan/design-principles.md`, `uianatomy` MCP via `uianatomy-mcp` Skill, `docs/src/pages/agent-skills.astro` |
...
| Hosted Storybook-MCP-Endpoints (3 Docs-Tools) per `.mcp.json` | **Lokale MCP-Konfig-Snippets** für Tag 2 Block 3: `http://localhost:4401/mcp`-Eintrag, der die React-Teilnehmer-Maschine an die Test-Tools koppelt; Hinweis für Angular/Vue auf `nx test <lib> --watch`-Loop |
...
4. Test-Loop grün: React → `test-run` MCP-Call passes; Angular/Vue → `nx test <lib>` passes
...
- Skill = Anleitung, die Claude bei passender Aufgabe lädt — `figma-workspace-architect` und `atelier-design` begleiten gleich Block 1
