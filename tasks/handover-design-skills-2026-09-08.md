...
1. **`figma-console` MCP is disconnected.** It dropped mid-session when the description
   ...
   Reconnect with `/mcp` before anything Figma-shaped. Everything below marked _needs
   Bridge_ waits on this.
2. **All 37 parity records are DRIFT** (`npm run check:parity` exit 1, 37 blockers).
   ...
   repo-wide by its own P0, and `check:parity` stays red until a re-verify sweep. _Needs
   Bridge._
...
2. **`artboard-bridge` iteration 3** — the write path P1–P7 has never run. Needs one
   ...
   description (iteration 2's verdict on it is anachronistic — see below). _Needs Bridge
   for the parity half._
...
- **Scratch Claude Design project `44481d29-1041-4aa0-adf0-cf59028016d7`** ("Atelier —
  ...
  `AtlBadge.dc.html` + `support.js` — the baseline run's sheet of _unverified_ AtlBadge
...
- **The iteration-2 governance verdict is anachronistic.** The grader judged that run
  against the description carve-out I added _after_ it ran. The run's reading was
...
- **Eval harness facts** (both skills' iteration records repeat these): agent worktrees
  are cut from the last _pushed_ commit and carry files uncommitted at creation time;
