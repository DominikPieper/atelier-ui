...
- **Governance is a precondition, not a footnote.** `/design-sync` uploads component source and rendered previews to claude.ai/design; Anthropic's own admin guide states uploaded assets are stored persistently under enterprise retention with no data-residency support, and Claude Design is default-OFF on Enterprise. Rehearsing on Atelier's own OSS library is the safe case. Any scenario where a participant pushes their **employer's** design system is a data-processing decision: per Conciso policy that goes to the DSB (Michael Didion / Harry Ursinus, datenschutz@conciso.de) and, for org enablement, the ISB (Daniel Naczinski), **before** it appears on an agenda a client sees.
