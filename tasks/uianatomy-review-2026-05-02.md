...
- **Action-group sub-anatomy duplicates across Card, Alert, Toast, Dialog, Drawer.** Already noted as a uianatomy-side issue (§Content quality), but the same drift exists _inside this DS_: each component’s `actions` slot has slightly different padding/alignment expectations. Extract a shared `LlmActionGroupSpec` and consume from each.
