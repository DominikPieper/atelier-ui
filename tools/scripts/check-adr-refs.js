...
const TRIGGER_WORDS = /\b(revises|revised|corrects|corrected|amends|amended|supersedes|superseded)\b/i;
const fileText = new Map(adrFiles.map((f) => [f, fs.readFileSync(path.join(ADR_DIR, f), 'utf8')]));

function adrNumbersIn(str, excludeNum) {
  const nums = new Set();
  for (const m of str.matchAll(/ADR-(\d{4})/gi)) if (m[1] !== excludeNum) nums.add(m[1]);
  for (const m of str.matchAll(/(\d{4})-[a-z0-9-]+\.md/g)) if (m[1] !== excludeNum) nums.add(m[1]);
  return nums;
}
...
for (const bFile of adrFiles) {
  ...
  const bDate = dateMatch ? dateMatch[1].trim().replace(/^["']|["']$/g, '') : null;

  const record = (str) => {
    for (const a of adrNumbersIn(str, bNum)) claims.set(`${a}:${bNum}`, { a, b: bNum, bFile, bDate });
  };
  ...
  if (sourcesMatch) {
    ...
    for (const m of sourcesMatch[1].matchAll(lineRe)) {
      const [, aNum, paren] = m;
      if (aNum !== bNum && TRIGGER_WORDS.test(paren)) claims.set(`${aNum}:${bNum}`, { a: aNum, b: bNum, bFile, bDate });
    }
  }
...
for (const { a, b, bFile, bDate } of claims.values()) {
  ...
  const named = new RegExp(`ADR-${b}\\b`).test(aText) || new RegExp(`${b}-[a-z0-9-]+\\.md`).test(aText);
