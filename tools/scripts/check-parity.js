...
function loadJson(file, label) {
  ...
  try {
    ...
    console.error(
      `✗ [${label}] ${path.relative(ROOT, file)} is not valid JSON: ${err.message}`,
    );
    process.exit(1);
  }
}
...
if (!Array.isArray(snapshot.components) || snapshot.components.length === 0) {
  console.error(
    `✗ [SNAPSHOT] ${path.relative(ROOT, SNAPSHOT_FILE)} has no components. Run npm run figma:snapshot.`,
  );
  process.exit(1);
}
...
for (const comp of snapshot.components) {
  ...
  if (!moduleName) {
    ...
    if (!exportedSpecs.has(specName)) {
      ...
      warning(
        'MAP',
        `${selector}: ${specName} is exported and is not in NON_COMPONENT_SPECS, but has no COMPONENT_METADATA_REGISTRY entry — so parity inputs cannot be located. Add the entry.`,
      );
    }
    continue;
  }
  ...
  if (!rec) {
    warning(
      'UNVERIFIED',
      `${selector}: never design-parity-verified. Run figma_check_design_parity (node ${comp.nodeId}), then: npm run parity:record -- --component ${selector}`,
    );
    continue;
  }
  recorded++;
  ...
  if (rec.inputsHash !== hash) {
    ...
    if (redesignPhase) {
      ...
      warning(
        'DRIFT',
        `${msg} — owed, not blocking: redesign phase (banner above).`,
      );
    } else if (REPORT_ONLY) {
      drifted.push(selector);
      warning(
        'DRIFT',
        `${msg} — owed, not blocking here: report mode (banner above).`,
      );
    } else {
      blocker('DRIFT', msg);
    }
  ...
  if (rec.figmaNodeId && comp.nodeId && rec.figmaNodeId !== comp.nodeId) {
    warning(
      'NODE',
      `${selector}: recorded Figma node ${rec.figmaNodeId} != snapshot node ${comp.nodeId}. The master may have been renumbered; re-verify to be safe.`,
    );
  }
}
...
function report() {
  ...
  const head =
    ...
    (untracked.length
      ? `; ${untracked.length} intentionally untracked (${untracked.join(', ')})`
      : '');
