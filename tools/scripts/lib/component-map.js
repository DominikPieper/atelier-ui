...
function maps() {
  ...
  const nonComponentSpecs = new Set(
    [
      ...fs
        .readFileSync(METADATA_INDEX, 'utf8')
        .matchAll(/'(Atl\w+)',?\s*\/\/[^\n]*/g),
    ]
      .map((m) => m[1])
      .filter((name) => {
        const block = /export const NON_COMPONENT_SPECS[^;]+;/s.exec(
          fs.readFileSync(METADATA_INDEX, 'utf8'),
        );
  ...
  const exportedSpecs = new Set(
    [
      ...fs
        ...
        .matchAll(/export interface (Atl\w*Spec)\b/g),
    ].map((m) => m[1]),
  );
